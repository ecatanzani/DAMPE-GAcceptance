...
void generate_coordinate(Double_t X[],TRandom3 &r_gen) {
    X[0] = r_gen.Uniform(-Ldet/2,Ldet/2);
    X[1] = r_gen.Uniform(-Ldet/2,Ldet/2);
    X[2] = Hdet/2;
}

void generate_theta_phi(Double_t &theta,Double_t &phi,TRandom3 &r_gen,TH1D &cos2theta_gen) {
    Double_t cos2theta = 0;
    phi = r_gen.Uniform(phi_min,phi_max);
    cos2theta = r_gen.Uniform(cos2_theta_min,cos2_theta_max);
    theta = acos(sqrt(cos2theta));
    cos2theta_gen.Fill(cos2theta);
